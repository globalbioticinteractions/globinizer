...
install_deps() {
  ...
  then
    ...
    curl --silent -L https://github.com/jgm/pandoc/releases/download/2.19.2/pandoc-2.19.2-1-amd64.deb > pandoc.deb && sudo install -q ./pandoc.deb
...
_EOF_
  tsv2csv\
  | pandoc --self-contained --metadata title=${REPO_NAME} --css=styling.css --to=html5 --from=csv -o -
}
